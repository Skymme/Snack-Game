 ...
 */
function isBlockingBorderForPlayer(raw, state) {
...
export const snakeContract = {
    getLegalMoves(state) {
        ...
        return directions
            ...
            if (isBlockingBorderForPlayer(raw, state))
                return false;
            // mur bloquant si pas gameOver
            const wall = isWall(wrapped, state);
            if (wall && !state.config.gameOverOn.includes("wall"))
                return false;
    ...
    applyMove(state, move) {
        ...
        // on l'ignore (pas de déplacement). (Le joueur doit bouger, mais ici c'est garde-fou.)
        {
            ...
            if (isBlockingBorderForPlayer(raw, newState))
                return newState;
            if (isWall(wrapped, newState) && !newState.config.gameOverOn.includes("wall"))
                return newState;
        }
        // 1) Joueur
