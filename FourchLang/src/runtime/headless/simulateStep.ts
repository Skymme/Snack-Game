...
import type { PlayableGameInit } from "../core/playableTypes.js";
...
interface SimulateOptions {
    outDir: string;
    aiSpec: string;    // "random" | "greedy" | "minimax:2"
    seed: number;
}
...
export async function simulateStep(opts: SimulateOptions) {
    const jsonPath = path.join(opts.outDir, "game.json");
    const raw = fs.readFileSync(jsonPath, "utf-8");
    const init = JSON.parse(raw) as PlayableGameInit;
    ...
    fs.writeFileSync(
        path.join(opts.outDir, "next_state.json"),
        JSON.stringify(out, null, 2),
        "utf-8"
    );
}

function pickMoveByAiSpec(
    state: GameState,
    actorId: string,
    spec: string
) {
